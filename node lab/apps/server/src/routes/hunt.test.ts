...
describe('Hunt API', () => {
  ...
  describe('Discovery Flow', () => {
    ...
    it('successfully discovers an undiscovered bug', async () => {
      ...
      (prismaMock.bug.findFirst as jest.Mock).mockResolvedValue({ id: 'bug-1', targetSystem: 'SERVER', targetTeamId: 'team-beta', status: 'DISCOVERED' });
      ...
      expect(prismaMock.bug.updateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: { targetSystem: 'SERVER', targetTeamId: 'team-beta', status: 'PLANTED' }
      }));
  ...
  describe('Claim Flow', () => {
    ...
    it('successfully claims a discovered bug', async () => {
      ...
      expect(res.status).toBe(200);
      expect(prismaMock.bug.updateMany).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 'bug-1', targetTeamId: 'team-beta', status: 'DISCOVERED' }
      }));
      expect(prismaMock.user.update).toHaveBeenCalledWith(expect.objectContaining({
        data: { huntScore: { increment: 10 } }
      }));
    ...
    it('rejects double claiming atomically', async () => {
      (prismaMock.bug.updateMany as jest.Mock).mockResolvedValue({ count: 0 });
      (prismaMock.bug.findUnique as jest.Mock).mockResolvedValue({ id: 'bug-1', targetTeamId: 'team-beta', status: 'CLAIMED' });
  ...
  describe('Solve Flow', () => {
    ...
    it('successfully resolves a claimed bug', async () => {
      (prismaMock.bug.updateMany as jest.Mock).mockResolvedValue({ count: 1 });
      
      const res = await request(app).post('/hunt/bugs/bug-1/solve').set('Authorization', `Bearer ${token}`).send({ solution: 'Correct' });
    ...
    it('rejects resolving a bug claimed by someone else', async () => {
      (prismaMock.bug.updateMany as jest.Mock).mockResolvedValue({ count: 0 });
      (prismaMock.bug.findUnique as jest.Mock).mockResolvedValue({ id: 'bug-1', claimedByUserId: 'user-2', status: 'CLAIMED' });
